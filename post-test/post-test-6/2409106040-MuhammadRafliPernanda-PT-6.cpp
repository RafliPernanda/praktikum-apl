...
void sortBySumberDescending(Catatan* catatan, int totalCatatan) {
    for (int currentIndex = 0; currentIndex < totalCatatan - 1; currentIndex++) {
        ...
        for (int compareIndex = currentIndex + 1; compareIndex < totalCatatan; compareIndex++) {
            if (catatan[compareIndex].sumber > catatan[indexSumberTerbesar].sumber) {
...
void sortByJumlahAscending(Catatan* catatan, int totalCatatan) {
    for (int passIndex = 0; passIndex < totalCatatan - 1; passIndex++) {
        ...
        for (int compareIndex = 0; compareIndex < totalCatatan - passIndex - 1; compareIndex++) {
...
void menuUtama(Catatan* catatan, int* totalCatatan) {
    ...
    while (true) {
        ...
        switch (pilihan) {
            ...
            case 8: menuSorting(catatan, totalCatatan); break;
            case 9: cout << "\nLogout berhasil!\n\n"; return;
